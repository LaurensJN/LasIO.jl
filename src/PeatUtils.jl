...
const gdt_lookup = Dict{DataType, UInt32}(
    UInt8 => GDAL.GDT_Byte,
...
const type_lookup = Dict{UInt32, DataType}(
    GDAL.GDT_Byte => UInt8,
...
function pointsample(band::Ptr{GDAL.GDALRasterBandH}, geotransform::Vector{Float64}, x::AbstractFloat, y::AbstractFloat)
    ...
    # TODO this should work with any GDAL data type (GDT)
    valref = Ref(0x00) # black
    # valref = Ref(0xff) # white

    nxoff = Int(fld(x - x_min, x_res))
    nyoff = Int(fld(y - y_max, y_res))

    GDAL.rasterio(band, GDAL.GF_Read, nxoff, nyoff, 1, 1,
                  valref, 1, 1, GDAL.GDT_Byte, 0, 0)
    valref[]
end
...
function read_raster(fname::String)
    ...
    wkt = GDAL.getprojectionref(dataset)
    epsg = wkt2epsg(wkt)

    GDAL.close(dataset)
    GDAL.destroydrivermanager()

    A', x_min, y_max, cellsize, epsg, nodata
end
